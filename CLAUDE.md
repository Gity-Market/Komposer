...
Current statuses: **SPEC-0001–0004 are Implemented** (Phases 1–2: wire format, node catalog, shared model/serialization, Android rendering pipeline). **SPEC-0005 (modifier system, Phase 3) is Implemented** — merged to `master` via #9 (`f542d02`); shared modifier models, serialization, the full `commonTest` round-trip/validation suite, and the Android fold/scope/renderer changes are all in. *Device/`assembleDebug`/`lint` acceptance (SPEC-0005 §10) is the one remaining gate; it hasn't been run in this environment's Google-Maven access.*
